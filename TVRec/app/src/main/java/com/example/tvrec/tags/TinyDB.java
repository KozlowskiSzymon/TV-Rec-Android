...
return newList;
	...
	public ArrayList<Tag> getListTags(String key){
		Gson gson = new Gson();
		...
		for(String jObjString : objStrings){
			Tag value  = gson.fromJson(jObjString,  Tag.class);
			objects.add(value);
		}
		return objects;
	}
...
*/
	...
	public void putListTags(String key, ArrayList<Tag> objArray){
		checkForNullKey(key);
		Gson gson = new Gson();
		ArrayList<String> objStrings = new ArrayList<>();
		for(Tag obj : objArray){
			objStrings.add(gson.toJson(obj));
		}
		putListString(key, objStrings);
	}
